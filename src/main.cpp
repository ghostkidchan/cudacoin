...
#if defined(NDEBUG)
# error "Cudacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9b06086feb1a834e923ac62e956eac6ac3cd6acad0a37fb2c2c974a3861918c6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 8); // Cudacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cudacoin Signed Message:\n";
...
{
    // Cudacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Cudacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Cudacoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Cudacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cudacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cudacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b06086feb1a834e923ac62e956eac6ac3cd6acad0a37fb2c2c974a3861918c6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cudacoin is Now Launching!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0458d5c9f6b3541129183099fdf2ae53d545e5b0878a66592d6561e3859f765611b2fb49939647194258127ecc619ce7d012a02edb0263e4f2eea9115bf188b101") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519977016;
        ...
        assert(block.hashMerkleRoot == uint256("0xbd658ec4ecc3266d8bf6dd2e009bd69b59f66df61e5b7cff172c4e3d696c896b"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb2, 0xd8 }; // Cudacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CudacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CudacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CudacoinMiner(CWallet *pwallet)
{
    printf("CudacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cudacoin-miner");
    ...
    try { loop {
        ...
        printf("Running CudacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CudacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CudacoinMiner, pwallet));
}
